...
const OPS_ENV_UPDATE_AUDIT_LOG_FILE = "ops-env-update.log";
...
const MCP_WORKFLOW_TEMPLATE_CATALOG = [
    {
    ...
    },
    {
        ...
        uriTemplate: "biddersweet://templates/csp-image-loading-regression-check{?service,window_minutes,domain,image_path}",
        ...
        whatThisDoes: "Starts with production triage for CSP/image failures, then checks 401 deploy-window auth effects and security header config in repo.",
        mimeType: "application/json",
        parameters: [
            { name: "service", type: "string", required: true, description: "Render backend service name/id." },
    ...
    },
    {
        ...
        uriTemplate: "biddersweet://templates/deploy-readiness-gate{?service,service_a,service_b,max_allowed_drift,include_integration}",
    ...
    },
    {
        ...
        whatThisDoes: "Runs deploy-window 401 classification first, then triages auth failures and checks env drift for session/cookie keys.",
        mimeType: "application/json",
        parameters: [
            { name: "service", type: "string", required: true, description: "Render backend service name/id." },
    ...
    },
    {
        ...
        uriTemplate: "biddersweet://templates/incident-to-contract-followup{?service,window_minutes,endpoint,filter,max_allowed_drift}",
        name: "Incident To Contract Follow-up",
        description: "Turn a production incident into contract/smoke follow-up checks.",
        whatThisDoes: "Runs incident triage first, then validates route/openapi drift and smoke readiness to prevent repeat regressions.",
        mimeType: "application/json",
        parameters: [
            { name: "service", type: "string", required: true, description: "Render backend service name/id." },
...
function parseTemplateUri(uri) {
    try {
        ...
        if (parsed.protocol !== "biddersweet:")
            return null;
        if (parsed.hostname !== "templates")
            return null;
...
function resolveTemplateParameters(parameters, searchParams) {
    const values = {};
...
function compactTemplateArguments(input) {
    const entries = Object.entries(input).filter(([, value]) => value !== undefined);
    return Object.fromEntries(entries);
}
function buildWorkflowTemplateResource(uri) {
...
async function handleRepoListDir(input) {
    ...
    const entries = dirEntries
        .filter((entry) => !isProtectedPath(joinRelativePath(resolved.relative, entry.name)))
        .map((entry) => {
...
async function handleRepoTree(input) {
    ...
    const buildTree = async (fullPath, relPath, name, depth) => {
        if (shouldStop()) {
            truncated = true;
        ...
        const filtered = entries.filter((entry) => {
            if (entry.isDirectory() && SKIP_DIR_NAMES.has(entry.name))
                return false;
            if (isProtectedPath(joinRelativePath(relPath, entry.name)))
                return false;
            return true;
        });
        filtered.sort((a, b) => {
...
let renderSnapshotCache = null;
async function getRenderSnapshot() {
...
async function listRenderLogsForOrchestrator(input) {
    ...
    const filtered = logs.filter((log) => {
        ...
        if (input.filter && !message.includes(input.filter))
            return false;
        if (input.endpoint) {
            const pathValue = typeof log.path === "string" ? log.path : "";
            if (!pathValue.includes(input.endpoint) && !message.includes(input.endpoint))
                return false;
        }
        if (input.requestId) {
            const requestId = typeof log.requestId === "string" ? log.requestId : "";
            if (!requestId.includes(input.requestId) && !message.includes(input.requestId))
                return false;
        }
        return true;
    });
    return {
        logs: filtered.slice(0, Math.max(1, input.limit)).map((log) => ({
            ...
            statusCode: typeof log.statusCode === "number" || typeof log.statusCode === "string" ? log.statusCode : null,
    ...
    async function walk(current) {
        ...
        for (const entry of entries) {
            ...
            if (entry.isDirectory()) {
                ...
                if (isProtectedPath(relPathNormalized))
                    continue;
...
function joinRelativePath(base, next) {
    if (base === "." || base.length === 0)
        return normalizeRelativePath(next);
...
function enforceCapabilityMode(toolName) {
    const capability = readCapabilityMode();
    if (capability === "READ_WRITE")
        return null;
...
function summarizePatchArgs(args, includeExpectedSha) {
    const diff = typeof args.diff === "string" ? args.diff : null;
    const insert = args.insert && typeof args.insert === "object" ? args.insert : null;
    const replace = args.replace && typeof args.replace === "object" ? args.replace : null;
    const del = args.delete && typeof args.delete === "object" ? args.delete : null;
        ...
        : insert
            ...
            : replace
                ...
                : del
                    ? "delete"
                    : "unknown";
    const textBytes = (insert && byteLength(insert.text)) ||
...
function summarizeToolResult(toolName, result, isError) {
    ...
    if (toolName === "repo.format_patch") {
        return {
            ...
            files: payload.stats && typeof payload.stats === "object" ? payload.stats.files : undefined
        };
    }
    ...
    if (toolName === "dev.run_tests" || toolName === "dev.run_lint") {
        return {
            ok: payload.ok,
            hasRuby: Boolean(payload.results?.ruby),
            hasJs: Boolean(payload.results?.js)
        };
    }
    ...
    if (toolName === "js.workspace") {
        return {
            ...
            hasWorkspaces: Boolean(payload.workspaces?.enabled)
        };
    }
    if (isError) {
        return { error: summarizeAuditError(result) };
...
function summarizeAuditError(result) {
    if (!result || typeof result !== "object")
...
function truncateString(value) {
    if (value.length <= AUDIT_MAX_STRING)
        return value;
...
function looksLikeSecret(value) {
    ...
    if (/^[A-Fa-f0-9]{40,}$/.test(trimmed))
        return true;
    if (/^[A-Za-z0-9+/_-]{40,}={0,2}$/.test(trimmed))
        return true;
    return false;
}
